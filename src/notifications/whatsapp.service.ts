...
export class WhatsappService {
  ...
  async sendTemplate(
    ...
    if (process.env.ENVIRONMENT !== 'production') {
      this.logger.warn(
        `WhatsApp no enviado: Entorno no es producción (${process.env.ENVIRONMENT})`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `WhatsApp enviado a ${formattedPhone} (Template: ${templateName})`,
      );
    } catch (error) {
      const errorMsg = error.response
        ...
        : error.message;
      this.logger.error(
        `Error enviando WhatsApp a ${formattedPhone}: ${errorMsg}`,
      );
    }
  }

  async sendNotification(phone: string, message: string): Promise<void> {
    return await this.sendTemplate(phone, 'notificaciones_distribuidores', [message]);
  }
